...
import type { Kundenverwaltung, Motivkatalog, Auftragsverwaltung, Produktionsplanung, Rechnungsverwaltung, CreateKundenverwaltung, CreateMotivkatalog, CreateAuftragsverwaltung, CreateProduktionsplanung, CreateRechnungsverwaltung } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- PRODUKTIONSPLANUNG ---
  static async getProduktionsplanung(): Promise<Produktionsplanung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.PRODUKTIONSPLANUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Produktionsplanung[];
    return enrichLookupFields(records, 'produktionsplanung');
  }
  static async getProduktionsplanungEntry(id: string): Promise<Produktionsplanung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.PRODUKTIONSPLANUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Produktionsplanung;
    return enrichLookupFields([record], 'produktionsplanung')[0];
  }
  static async createProduktionsplanungEntry(fields: CreateProduktionsplanung) {
    return callApi('POST', `/apps/${APP_IDS.PRODUKTIONSPLANUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'produktionsplanung') });
  }
  static async updateProduktionsplanungEntry(id: string, fields: Partial<CreateProduktionsplanung>) {
    return callApi('PATCH', `/apps/${APP_IDS.PRODUKTIONSPLANUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'produktionsplanung') });
  }
  static async deleteProduktionsplanungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.PRODUKTIONSPLANUNG}/records/${id}`);
  }
