...
interface ProduktionsplanungDialogProps {
  open: boolean;
  onClose: () => void;
...
export function ProduktionsplanungDialog({ open, onClose, onSubmit, defaultValues, auftragsverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: ProduktionsplanungDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Produktionsplanung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'produktionsplanung');
      await onSubmit(clean as Produktionsplanung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "tatsaechliches_druckdatum": string | null, // YYYY-MM-DD\n  "drucker": LookupValue | null, // Verwendeter Drucker (select one key: "vertikaldrucker_1" | "vertikaldrucker_2" | "vertikaldrucker_3") mapping: vertikaldrucker_1=Vertikaldrucker 1, vertikaldrucker_2=Vertikaldrucker 2, vertikaldrucker_3=Vertikaldrucker 3\n  "zustaendiger_mitarbeiter": string | null, // Zuständiger Mitarbeiter\n  "druckstatus": LookupValue | null, // Druckstatus (select one key: "geplant" | "in_druck" | "druck_abgeschlossen" | "qualitaetspruefung" | "freigegeben" | "nacharbeit_erforderlich") mapping: geplant=Geplant, in_druck=In Druck, druck_abgeschlossen=Druck abgeschlossen, qualitaetspruefung=Qualitätsprüfung, freigegeben=Freigegeben, nacharbeit_erforderlich=Nacharbeit erforderlich\n  "qualitaetspruefung_bestanden": boolean | null, // Qualitätsprüfung bestanden\n  "produktionsanmerkungen": string | null, // Anmerkungen zur Produktion\n  "produktionsnummer": string | null, // Produktionsnummer\n  "auftrag": string | null, // Display name from Auftragsverwaltung (see <available-records>)\n  "geplantes_druckdatum": string | null, // YYYY-MM-DD\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Produktionsplanung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Produktionsplanung bearbeiten' : 'Produktionsplanung hinzufügen';

  const fieldBlocks: Record<string, React.ReactNode> = {
    'tatsaechliches_druckdatum': (
      <div key="tatsaechliches_druckdatum" className="space-y-1.5">
        <Label htmlFor="tatsaechliches_druckdatum">Tatsächliches Druckdatum</Label>
        <DatePicker
          id="tatsaechliches_druckdatum"
          placeholder="Tatsächlicher Druckdatum?"
          mode="date"
    ...
    'drucker': (
      <div key="drucker" className="space-y-1.5">
    ...
    'zustaendiger_mitarbeiter': (
      <div key="zustaendiger_mitarbeiter" className="space-y-1.5">
    ...
    'druckstatus': (
      <div key="druckstatus" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="druckstatus"><SelectValue placeholder="Aktueller Druckfortschritt?" /></SelectTrigger>
          <SelectContent>
    ...
    'qualitaetspruefung_bestanden': (
      <div key="qualitaetspruefung_bestanden" className="space-y-1.5">
    ...
    'produktionsanmerkungen': (
      <div key="produktionsanmerkungen" className="space-y-1.5">
        <Label htmlFor="produktionsanmerkungen">Anmerkungen zur Produktion</Label>
        <Textarea
          id="produktionsanmerkungen"
          placeholder="Besonderheiten, Fehler, Optimierungen..."
    ...
    'auftrag': (
      <div key="auftrag" className="space-y-1.5">
        ...
        <Combobox
          id="auftrag"
          placeholder="Welcher Auftrag wird produziert?"
    ...
    'geplantes_druckdatum': (
      <div key="geplantes_druckdatum" className="space-y-1.5">
        <Label htmlFor="geplantes_druckdatum">Geplantes Druckdatum</Label>
        <DatePicker
          id="geplantes_druckdatum"
          placeholder="Wann ist der Druck geplant?"
          mode="date"
  ...
  const FIELD_LABELS: Record<string, string> = {"tatsaechliches_druckdatum": "Tatsächliches Druckdatum", "drucker": "Verwendeter Drucker", "zustaendiger_mitarbeiter": "Zuständiger Mitarbeiter", "druckstatus": "Druckstatus", "qualitaetspruefung_bestanden": "Qualitätsprüfung bestanden", "produktionsanmerkungen": "Anmerkungen zur Produktion", "produktionsnummer": "Produktionsnummer", "auftrag": "Auftrag", "geplantes_druckdatum": "Geplantes Druckdatum"};
