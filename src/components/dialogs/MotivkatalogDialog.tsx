...
export function MotivkatalogDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: MotivkatalogDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "beschreibung": string | null, // Beschreibung\n  "kategorie": LookupValue | null, // Kategorie (select one key: "natur_landschaft" | "stadtansichten" | "abstrakt" | "tiere" | "architektur" | "kunst_illustration" | "sonstiges") mapping: natur_landschaft=Natur & Landschaft, stadtansichten=Stadtansichten, abstrakt=Abstrakt, tiere=Tiere, architektur=Architektur, kunst_illustration=Kunst & Illustration, sonstiges=Sonstiges\n  "max_breite_cm": number | null, // Maximale Druckbreite (cm)\n  "max_hoehe_cm": number | null, // Maximale Druckhöhe (cm)\n  "material": LookupValue[] | null, // Verfügbare Materialien (select one or more keys: "leinwand" | "fotopaier" | "acrylglas" | "aluminium_dibond" | "tapete" | "pvc_plane") mapping: leinwand=Leinwand, fotopaier=Fotopapier, acrylglas=Acrylglas, aluminium_dibond=Aluminium-Dibond, tapete=Tapete, pvc_plane=PVC-Plane\n  "preis_pro_qm": number | null, // Preis pro m² (€)\n  "aktiv": boolean | null, // Motiv aktiv (im Angebot verfügbar)\n  "motivname": string | null, // Motivname\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Bilddetails, Stil, Inspirationsquelle..."
    ...
    'kategorie': (
      <div key="kategorie" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="kategorie"><SelectValue placeholder="Welche Kategorie passt?" /></SelectTrigger>
          <SelectContent>
    ...
    'max_breite_cm': (
      <div key="max_breite_cm" className="space-y-1.5">
        <Label htmlFor="max_breite_cm">Maximale Druckbreite (cm)</Label>
        <Input
          id="max_breite_cm"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'max_breite_cm')}
          placeholder="z. B. 150"
          value={fields.max_breite_cm !== undefined ? fields.max_breite_cm : (computedValues['max_breite_cm'] ?? '')}
          onChange={e => setFields(f => ({ ...f, max_breite_cm: clampNumberValue(formEnhancements, 'max_breite_cm', e.target.value) }))}
        />
      </div>
    ),
    'max_hoehe_cm': (
      <div key="max_hoehe_cm" className="space-y-1.5">
        <Label htmlFor="max_hoehe_cm">Maximale Druckhöhe (cm)</Label>
        <Input
          id="max_hoehe_cm"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'max_hoehe_cm')}
          placeholder="z. B. 200"
          value={fields.max_hoehe_cm !== undefined ? fields.max_hoehe_cm : (computedValues['max_hoehe_cm'] ?? '')}
          onChange={e => setFields(f => ({ ...f, max_hoehe_cm: clampNumberValue(formEnhancements, 'max_hoehe_cm', e.target.value) }))}
        />
      </div>
    ),
    'material': (
  ...
  const FIELD_LABELS: Record<string, string> = {"beschreibung": "Beschreibung", "kategorie": "Kategorie", "max_breite_cm": "Maximale Druckbreite (cm)", "max_hoehe_cm": "Maximale Druckhöhe (cm)", "material": "Verfügbare Materialien", "preis_pro_qm": "Preis pro m² (€)", "vorschaubild": "Vorschaubild", "aktiv": "Motiv aktiv (im Angebot verfügbar)", "motivname": "Motivname"};
