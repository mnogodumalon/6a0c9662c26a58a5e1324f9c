...
export function KundenverwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: KundenverwaltungDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
      ...
      <div key="kundentyp" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kundentyp) === 'privatkunde'}
            onClick={() => setFields(f => ({ ...f, kundentyp: (lookupKey(f.kundentyp) === 'privatkunde' ? undefined : 'privatkunde') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kundentyp) === 'privatkunde'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kundentyp) === 'geschaeftskunde'}
            onClick={() => setFields(f => ({ ...f, kundentyp: (lookupKey(f.kundentyp) === 'geschaeftskunde' ? undefined : 'geschaeftskunde') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kundentyp) === 'geschaeftskunde'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'anmerkungen': (
      <div key="anmerkungen" className="space-y-1.5">
        <Label htmlFor="anmerkungen">Anmerkungen zum Kunden</Label>
        <Textarea
          id="anmerkungen"
          placeholder="Besondere Wünsche, bevorzugte Kontaktzeiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "unternehmen": "Unternehmen / Firma", "kundentyp": "Kundentyp", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "ort": "Ort", "land": "Land", "anmerkungen": "Anmerkungen zum Kunden"};
